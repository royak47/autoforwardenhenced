...
async def pub_(bot, message):
    ...
    # Get extra configs for views filter
    configs = await db.get_configs(user)
    min_views = int(configs.get('min_views', 0) or 0)
    top_n = int(configs.get('top_n', 0) or 0)
    ...
    if locked:
        try:
          MSG = []
          candidates = []  # for top_n mode
          pling = 0
          await edit(m, 'Progressing', 10, sts)
          print(f"Starting Forwarding... From:{sts.get('FROM')} To:{sts.get('TO')} Limit:{sts.get('limit')} Skip:{sts.get('skip')} MinViews:{min_views} TopN:{top_n}")
          ...
          async for message in client.iter_messages(
            ...
            offset=int(sts.get('skip')) if sts.get('skip') else 0
                ...
                # If top_n mode → collect first, forward later
                if top_n > 0:
                    candidates.append((msg_views, message))
                    continue
          ...
          if top_n > 0 and candidates:
              ...
              await send(client, user, f"<b>🏆 Top {len(top_messages)} messages selected (highest views)</b>")
...
async def edit(msg, title, status, sts):
   ...
   if status in ["cancelled", "completed"]:
      button.append(
         [InlineKeyboardButton('Support', url='https://t.me/scripthub0'),
         InlineKeyboardButton('Updates', url='https://t.me/scripthub00')]
         )
   else:
